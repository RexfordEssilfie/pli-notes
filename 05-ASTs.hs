...
-- collection of such passes a pipeline, an example of which is shown below:
...
--    first pass of a compiler is called the lexer which turns our file (really,
--    a long string) into a collection of tokens or a lexical stream. In many
--    languages, tokens constitute logical units of text separated by
--    whitespace.
-- 2. Next, the compiler takes the lexical stream and performs a pass called
--    parsing where the linear collection of tokens are turned into a tree-like
--    data structure called an _abstract syntax tree_ (AST). The AST is named
--    as such because it captures essential information about the structure of
--    the program (e.g., an assignment statement) and ignores the inessential
--    parts (semicolons at the end of a statement).
...
--    program).
-- 4. From the AST, the compiler then lowers the tree into some sort of
...
-- We'll define the language with a grammar describing the set of possible
-- language constructs and the legal ways of forming a program from these
-- constructs. A grammar consists of a collection of productions which describe
...
-- form an expression by choosing among two alternatives. The first says we any
...
-- that appeared in the concrete syntax of the language is made implicit by
-- the tree-like nature of the abstract syntax tree.
...
-- Pretty printing is an example of a pass over the AST, producing a single
-- string as a result.  Another example of a pass is interpretation which
-- evaluates the program encoded by the AST down to a final value. In essence,
-- interpretation allows us to run programs without actually compiling the
-- program down to some lower-level representation. This is convenient when we
-- are interested in quickly prototyping language features without having to
-- fill out the full compilation pipeline for each feature we add to the
-- language.
